...
//  processor
//
//  PiDiNet edge probability map via dynamically loaded Core ML model.
//
...
/**
 PiDiNet edge-map provider (Android `pidinet.tflite` parity).

 Letterbox 384×384, pad `#727272` (RGB 114,114,114), ImageNet normalize, NCHW float32.
 */
final class LMPidinetModelProvider: @unchecked Sendable {
    static let shared = LMPidinetModelProvider()

    static let inputSize = 384
    private static let modelBaseName = "PiDiNet"
    ...
    /// Whether the PiDiNet model is already resident in memory.
    var isLoaded: Bool {
        lock.lock()
        defer { lock.unlock() }
        return model != nil
    ...
    /// Whether a bundled PiDiNet Core ML package is present.
    ...
    /**
     Asynchronously loads PiDiNet if needed (Stage A preload).
    ...
    func ensureModelLoadedAsync() async throws {
        ...
        guard let url = bundledModelURL() else {
            throw PidinetError.modelNotFound
        }
        ...
        if model == nil {
            model = loaded
            LMLogger.log("✅ PiDiNet model loaded (\(Self.inputSize)x\(Self.inputSize), async)")
        }
        lock.unlock()
    }

    /// Loads the PiDiNet model from bundle if not already loaded (predict-path fallback).
    func ensureModelLoaded() throws {
        ...
        LMLogger.log("✅ PiDiNet model loaded (\(Self.inputSize)x\(Self.inputSize))")
    }
    ...
    /**
     Predicts a 384×384 edge probability map (row-major `[Float]` of length `384*384`).

     Returns `nil` on load / preprocess / inference failure.
     */
    func predictEdgeMap(for image: UIImage) -> [Float]? {
        do {
            try ensureModelLoaded()
        } catch {
            LMLogger.log("❌ PiDiNet model load failed: \(error.localizedDescription)")
            return nil
        }
        ...
        guard let input = makeLetterboxImagenetNchw(from: image, size: Self.inputSize) else {
            LMLogger.log("❌ PiDiNet preprocess failed")
            return nil
        }
        ...
        do {
            ...
            guard let values = extractOutputMap(from: output, model: loadedModel, size: Self.inputSize) else {
                LMLogger.log("❌ PiDiNet output extract failed")
                return nil
            }
            return values
        } catch {
            LMLogger.log("❌ PiDiNet inference failed: \(error.localizedDescription)")
            return nil
        }
    }
    ...
    /// Flattens model output into a clipped [0, 1] edge map of `size*size`.
    private func extractOutputMap(from output: MLFeatureProvider, model: MLModel, size: Int) -> [Float]? {
        let outputName = model.modelDescription.outputDescriptionsByName.keys.first ?? "fused"
    ...
    enum PidinetError: Error, LocalizedError {
        case modelNotFound

        var errorDescription: String? {
            switch self {
            case .modelNotFound:
                return "\(LMPidinetModelProvider.modelBaseName).mlmodelc / .mlpackage not found in bundle"
            }
        }
    }
}
